...
{
	char* fname=(char*)(filename+1); // Variable sin el '/'	
	
	//Buscamos si el archivo esta en nuestro directorio usando findFileByName
	
	int nodoID = findFileByName(&myFileSystem, fname); // nodoID identificador del i-nodo
	 
	if (nodoID == -1) {
		return -1; // No existe el fichero
	}

	myFileSystem.directory.numFiles -= 1; // Decrementar numero de archivos en el directorio.
	myFileSystem.directory.files[nodoID].freeFile = true;
	updateDirectory(&myFileSystem); //Actualiza el directorio
	
	int copyNodeIDx = myFileSystem.directory.files[nodoID].nodeIdx; // Hacer una copia de nodoIDx del directorio que nos sirve para acceder al array de nodos.
	NodeStruct* inode=myFileSystem.nodes[copyNodeIDx];
	
	//Eliminar los bloques asociados a cada fichero
	...
	inode->numBlocks=0;
	inode->fileSize=0;
	inode->freeNode=true; // Borra el nodo del array

	updateNode(&myFileSystem,copyNodeIDx,inode); // Actualiza el nodo
	
	free(inode);
	myFileSystem.nodes[copyNodeIDx]=NULL;
...
static int my_read (const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *ffi)
{
	...
	NodeStruct *node = myFileSystem.nodes[ffi->fh]; 
